...
with open('../data/episodeLevelData.jsonl', 'r') as f:
    for line in f:
        ...
        if isinstance(values_to_insert[28], list):
            ...
            values_to_insert[28] = json.dumps(values_to_insert[28])
        ...
        if isinstance(values_to_insert[34], list):
            ...
            values_to_insert[34] = json.dumps(values_to_insert[34])

        if isinstance(values_to_insert[36], dict):
            values_to_insert[36] = json.dumps(values_to_insert[36])

        # Ensure all values are properly handled as strings or None
        for i in range(len(values_to_insert)):
            if isinstance(values_to_insert[i], (list, dict)):
        ...
        # Insert the record into the database (excluding epID, which auto-increments)
        cursor.execute('''
